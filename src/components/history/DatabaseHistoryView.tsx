...
// refetch off, so this listens to `db://update` instead (see `lib/db.ts`).

import { useCallback, useEffect, useState } from "react";
import { dbHistory, dbRevertTo, onDatabasesChanged, type DbHistoryEntryDto } from "../../lib/db";
...
export function DatabaseHistoryView({ databaseId }: DatabaseHistoryViewProps) {
  ...
  const restore = useCallback(() => {
    if (!pending) return;
    setRestoring(true);
    dbRevertTo(databaseId, pending.token)
