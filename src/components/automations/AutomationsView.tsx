...
import { createAutomation, deleteAutomation, listAutomations, setAutomationEnabled, updateAutomation } from "../../lib/automations";
import { createSkill, deleteSkill, listSkills, updateSkill } from "../../lib/skills";
import type { Automation, Skill } from "../../lib/types";
import { AutomationIcon, BackIcon, CloseIcon } from "../icons";
...
export function AutomationsView({ faded = false, transitionMs = 120, onClose, drawerStyle }: AutomationsViewProps) {
  ...
  return (
    <div className={`automation-view${onClose ? " automation-view--drawer" : ""}`} style={style}>
      <div className="automation-view__header">
        {screen.kind === "list" ? (
          <AutomationIcon size={16} />
        ) : (
          <button type="button" className="agent-panel__close" onClick={goBack} aria-label="Back">
            <BackIcon size={16} />
          </button>
        )}
        <span className="automation-view__label">{screenTitle(screen)}</span>
        {onClose && (
          <button type="button" className="automation-view__close" onClick={onClose} aria-label="Close automations">
            <CloseIcon size={16} />
          </button>
        )}
      </div>
