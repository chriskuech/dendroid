...
describe("AgentPanel", () => {
  it("shows a configure prompt instead of the composer when no command is set", () => {
    render(<AgentPanel cwd="/tmp/ws" agentSettings={{ command: "", args: "" }} open onClose={vi.fn()} />);
  ...
  it("connects and sends a prompt on first message, showing the user's bubble immediately", async () => {
    ...
    render(<AgentPanel cwd="/tmp/ws" agentSettings={agentSettings} open onClose={vi.fn()} />);
    await user.type(screen.getByPlaceholderText(/message the agent/i), "hello there{enter}");

    expect(screen.getByText("hello there")).toBeInTheDocument();
    await waitFor(() => expect(acp.startAgent).toHaveBeenCalledWith("/tmp/ws", agentSettings));
  ...
  it("calls onClose from the close button", async () => {
    const onClose = vi.fn();
    const user = userEvent.setup();
    render(<AgentPanel cwd="/tmp/ws" agentSettings={agentSettings} open onClose={onClose} />);
