...
// `lib/acp.ts`, same "vi.mock stands in for the Tauri IPC boundary"
// approach `DatabaseListView.test.tsx` uses for `lib/db.ts`.
//
// `lib/threads.ts` itself isn't mocked (its id-generation/default-title
// logic is worth exercising for real) but the `lib/settingsStore.ts` calls
...
vi.mock("../../lib/settingsStore", async () => {
  const actual = await vi.importActual<typeof import("../../lib/settingsStore")>("../../lib/settingsStore");
...
beforeEach(() => {
  ...
  vi.mocked(settingsStore.loadThreads).mockImplementation(async () => saved);
  vi.mocked(settingsStore.saveThreads).mockImplementation(async (threads) => {
    saved = threads;
  });
});
 ...
 * otherwise only ever visible to `lib/acp.ts` calls a given test may not
...
describe("AgentPanel", () => {
  ...
  it("connects and sends a prompt on first message, showing the user's bubble immediately", async () => {
    ...
    await waitFor(() => expect(acp.startAgent).toHaveBeenCalledWith(expect.any(String), "/tmp/ws", agentSettings, null));
    expect(acp.sendPrompt).toHaveBeenCalledWith(expect.any(String), "hello there");
  });
  ...
  it("passes the Local MCP URL to startAgent when it's enabled", async () => {
    ...
    await waitFor(() =>
      expect(acp.startAgent).toHaveBeenCalledWith(expect.any(String), "/tmp/ws", agentSettings, "http://127.0.0.1:7717/mcp"),
    );
  });
  ...
  it("shows a permission request and answers it, disabling the option once chosen", async () => {
    ...
    expect(acp.respondPermission).toHaveBeenCalledWith(expect.any(String), "req-1", { outcome: "selected", optionId: "allow" });
