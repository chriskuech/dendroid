...
import type { TableRowsDto } from "../../lib/db";
...
describe("DatabaseView", () => {
  ...
  it("editing a cell commits an UPDATE addressed by rowid", async () => {
    ...
    await waitFor(() =>
      expect(db.execSql).toHaveBeenCalledWith(
        "db-1",
  ...
  it("Add row inserts default values into the selected table", async () => {
    ...
    await waitFor(() => expect(db.execSql).toHaveBeenCalledWith("db-1", 'INSERT INTO "todos" DEFAULT VALUES', [], false));
  });
  ...
  it("deleting a row runs a DELETE addressed by rowid", async () => {
    ...
    await waitFor(() => expect(db.execSql).toHaveBeenCalledWith("db-1", 'DELETE FROM "todos" WHERE rowid = ?1', [1], false));
  });

  it("creating a table runs a quoted CREATE TABLE and selects it", async () => {
    const user = userEvent.setup();
    vi.mocked(db.listTables).mockResolvedValueOnce([]).mockResolvedValue([{ name: "people", columns: [] }]);
    vi.mocked(db.tableRows).mockResolvedValue({ columns: [], rows: [], totalRows: 0 });
    vi.mocked(db.execSql).mockResolvedValue(undefined);
    render(<DatabaseView database={database} onClose={vi.fn()} />);
    ...
    await waitFor(() => expect(db.execSql).toHaveBeenCalledWith("db-1", 'CREATE TABLE "people" ("name" TEXT)', [], false));
  });
