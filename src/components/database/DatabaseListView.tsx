...
// of sidebar content (see components/sidebar/Sidebar.tsx). Selecting a row
// opens that database in the main area, replacing the Editor — see
// Workspace.tsx's `selectedDatabaseId`.

import { useCallback, useEffect, useState, type CSSProperties } from "react";
import { createDatabase, deleteDatabase, listDatabases, onDatabasesChanged, type DatabaseDto } from "../../lib/db";
import { CloseIcon, DatabaseIcon, IncrementIcon, TrashIcon } from "../icons";
...
interface DatabaseListViewProps {
  ...
  /** Same "faded chrome" wiring TreeView/HistoryView apply to themselves —
   * see Sidebar.tsx. Only meaningful for the persistent (>=900px) sidebar. */
  faded?: boolean;
  transitionMs?: number;
  /** Present only in the <900px drawer variant — see Sidebar.tsx. */
  onClose?: () => void;
...
export function DatabaseListView({
  ...
  useEffect(() => {
    ...
    refresh();
    return onDatabasesChanged(refresh);
  }, [refresh]);
  ...
  }, [newName, creating, onSelect]);

  const handleDelete = useCallback(() => {
    if (!pendingDelete) return;
    setDeleting(true);
    deleteDatabase(pendingDelete.id)
  ...
  return (
    <div className={`database-list${onClose ? " database-list--drawer" : ""}`} style={style}>
      <div className="database-list__header">
        <DatabaseIcon size={16} />
        <span className="database-list__label">Databases</span>
        {onClose && (
          <button type="button" className="database-list__close" onClick={onClose} aria-label="Close databases">
            <CloseIcon size={16} />
          </button>
        )}
      </div>
      ...
      <div className="database-list__rows">
        {loading ? (
          ...
          databases.map((db) => (
            <div
              key={db.id}
              className={`database-row${db.id === selectedId ? " is-active" : ""}`}
              onClick={() => onSelect(db.id)}
            >
              <DatabaseIcon size={12} />
              <span className="database-row__name">{db.name}</span>
              <span
