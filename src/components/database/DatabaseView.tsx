...
// TABLE`, ...). Every mutation goes through `lib/db.ts`'s `execSql`, which
...
export function DatabaseView({ database, onClose }: DatabaseViewProps) {
  ...
  const refreshRows = useCallback(() => {
    ...
    tableRows(database.id, selectedTable, PAGE_SIZE, offset, sortColumn, sortDesc)
      .then((result) => {
        setRows(result);
        setError(null);
      })
      .catch((err: unknown) => setError(String(err)));
  }, [database.id, selectedTable, offset, sortColumn, sortDesc]);
  ...
  const runConsole = useCallback(() => {
    ...
    if (looksReadOnly(sql)) {
      queryDb(database.id, sql.replace(/;\s*$/, ""))
        .then((result) => {
    ...
    runExec(sql, [], true)
      ...
      .catch(() => {})
      .finally(() => setConsoleRunning(false));
  }, [consoleText, consoleRunning, database.id, runExec, refreshTables, refreshRows]);
