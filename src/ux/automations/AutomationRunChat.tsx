...
// thread's chat uses (ux/agent/timelineUpdates.ts) reconstructs the exact
...
function buildTimeline(run: AutomationRun): TimelineItem[] {
  ...
  for (const update of run.updates) {
    // `run.updates` entries mirror adapters/acp's `AcpUpdate` — untyped raw
...
export function AutomationRunChat({ automationId, runId }: AutomationRunChatProps) {
  ...
  }, [automationId, runId, automationsEngine]);
