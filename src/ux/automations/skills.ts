...
// mirrors ux/agent/threads.ts's shape closely on purpose. Every mutation re-syncs
...
// snapshot, not a live reference — see `adapters/automationsEngine`) or the
...
export async function listSkills(): Promise<Skill[]> {
  return settingsStore.loadSkills();
}

async function mutate(fn: (skills: Skill[]) => Skill[]): Promise<Skill[]> {
  const next = fn(await settingsStore.loadSkills());
  await settingsStore.saveSkills(next);
