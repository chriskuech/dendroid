...
// against a mocked `adapters/db` instead of a `DendroidDocument` instance
...
describe("DatabaseHistoryView", () => {
  it("shows an empty state when there's no history yet", async () => {
    vi.mocked(db.adapter.dbHistory).mockResolvedValue([]);
  ...
  it("lists entries, newest first, with 'Current' instead of Restore on the first one", async () => {
    vi.mocked(db.adapter.dbHistory).mockResolvedValue([entry("1", { message: "INSERT INTO t VALUES ('two')" }), entry("0")]);
    render(<DatabaseHistoryView databaseId="db-1" />);
  ...
  it("restoring an entry asks for confirmation before calling dbRevertTo", async () => {
    ...
    vi.mocked(db.adapter.dbHistory).mockResolvedValue([entry("1"), entry("0")]);
    vi.mocked(db.adapter.dbRevertTo).mockResolvedValue(undefined);
    render(<DatabaseHistoryView databaseId="db-1" />);
    ...
    expect(db.adapter.dbRevertTo).not.toHaveBeenCalled();

    await user.click(screen.getByRole("button", { name: /^roll back$/i }));
    await waitFor(() => expect(db.adapter.dbRevertTo).toHaveBeenCalledWith("db-1", "0"));
  });

  it("surfaces a fetch error instead of silently showing an empty list", async () => {
    vi.mocked(db.adapter.dbHistory).mockRejectedValue(new Error("database not found"));
    render(<DatabaseHistoryView databaseId="db-1" />);
