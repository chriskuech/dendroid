...
import { EncryptionIcon, QrKeyIcon } from "../../ui/icons";
...
export function EncryptionSection({ crdt }: EncryptionSectionProps) {
  ...
  async function openShowQr() {
    ...
    // storage (see `adapters/settingsStore`'s `loadEncryptionKeyText`,
    // OS-keychain-backed under Tauri) — the backend never hands raw key
    // material back out except right when a key is first created
    // (`generateEncryptionKey`'s return value).
