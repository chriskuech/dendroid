...
// `Skill` — mirrors lib/threads.ts's/lib/skills.ts's shape. Every mutation
// re-syncs the automation engine afterward (`syncAutomationsEngine`), same
// reasoning as lib/skills.ts's own mutate helper.

import { loadAutomations, saveAutomations } from "./settingsStore";
import type { Automation, AutomationDataTrigger, CronSchedule } from "./types";
...
async function mutate(fn: (automations: Automation[]) => Automation[]): Promise<Automation[]> {
  const next = fn(await loadAutomations());
